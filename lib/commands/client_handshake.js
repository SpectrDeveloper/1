...
function ClientHandshake(clientFlags) {
  ...
  this._lastAuthSwitchRequest = null;
  console.log(flagNames(clientFlags));
  console.log(flagNames(979919));
  Command.call(this);
}
...
ClientHandshake.prototype.calculatSha2AuthToken = function(authPluginData) {
  var authSha2 = require('../auth_sha2.js');
  return authSha2.calculateToken(this.password, authPluginData);
};
...
ClientHandshake.prototype.cachingSha2 = function(packet, connection) {
  console.log('here!');
  return null;
};
...
ClientHandshake.prototype.handshakeResult = function(packet, connection) {
  ...
  if (marker === 0xfe || marker === 1) {
    ...
    if (marker === 1) {
      ...
      authSwitchHandlerParams.pluginName = this._lastAuthSwitchRequest.pluginName;
    } else {
      ...
      this._lastAuthSwitchRequest = asr;
    }
    if (authSwitchHandlerParams.pluginName == 'mysql_native_password') {
      let authToken = this.calculateNativePasswordAuthToken(
        authSwitchHandlerParams.pluginData
      );
      connection.writePacket(
        new Packets.AuthSwitchResponse(authToken).toPacket()
      );
    } else if (authSwitchHandlerParams.pluginName == 'caching_sha2_password') {
      if (marker === 1) {
        //connection.writePacket(Packets.OK.toPacket());
        console.log(
          '=========================================================='
        );
        console.log(authSwitchHandlerParams.pluginData.toString());
        return ClientHandshake.prototype.cachingSha2;
        //return null;
      }
      let authToken = this.calculatSha2AuthToken(
        authSwitchHandlerParams.pluginData.slice(0, 20)
      );
      connection.writePacket(
        new Packets.AuthSwitchResponse(authToken).toPacket()
      );
      ...
      connection.emit(
        'error',
        new Error(
          `Server requires auth switch, but no auth switch handler provided for plugin ${
