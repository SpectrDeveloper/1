...
ClientHandshake.prototype.calculatSha2AuthToken = function(
  authPluginData
) {
  return authSha2.calculateToken(
    this.password,
    authPluginData
  );
};
  ...
  if (marker === 0xfe || marker === 1) {
    ...
    if (authSwitchHandlerParams.pluginName == 'mysql_native_password') {
      ...
      connection.emit(
        'error',
        new Error(
          `Server requires auth switch, but no auth switch handler provided for plugin ${authSwitchHandlerParams.pluginName}`
        )
      );
      return null;
    }
